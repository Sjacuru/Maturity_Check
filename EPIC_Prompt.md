...
## INPUT SPECIFICATION: PRD handoff block (`[PRD_HANDOFF_BLOCK]`)

***Read the PRD's `[PRD_HANDOFF_BLOCK]` first as the authoritative source for canonical in-scope IDs, personas, scope ceiling, and unresolved markers. Use the full PRD only to elaborate those in-scope items without expanding scope.***

The `[PRD_HANDOFF_BLOCK]` contains:
...
1. Locate and read the `[PRD_HANDOFF_BLOCK] ... [/PRD_HANDOFF_BLOCK]` block first and fully before processing the rest of the PRD
2. Extract the canonical_ids scope list: all FR-XXX and NFR-XXX IDs listed in the handoff block
...
**Critical Enforcement:** Every Epic you generate must correspond to at least one ID from the handoff block scope list. If you encounter a logical dependency beyond that scope, log it in "Open Questions / Out of Scope Dependencies" — do NOT create an Epic for it.

The handoff block defines canonical in-scope IDs, personas, assumptions, thresholds, and scope ceiling.
The full PRD may be used to elaborate Epic goals, story behavior, and acceptance criteria, provided no Epic, Story, or criterion introduces scope beyond the IDs listed in the handoff block.

***If the handoff block contains no unresolved [ASSUMPTION] or [THRESHOLD NEEDED] items, explicitly record “None carried forward from PRD”***

GATE CHECK: Before generating epics, verify:

1. The PRD contains a `[PRD_HANDOFF_BLOCK]` with at least one complete FR or NFR.
...
Do NOT rename, renumber, or merge IDs. Preserve exactly as written in the PRD (canonical IDs appear in `[PRD_HANDOFF_BLOCK]`).

**§1.1 — Small, Strategic Chunks (Planning Rule; Non-Negotiable)**
Every Epic must be decomposed so it can be delivered in **small, reviewable increments** (“chunks”) that are easy to track and verify.

- Each Epic must include a **Delivery Slices / Chunks** subsection containing **3–8 slices** ordered strategically (vertical value first).
- Each slice must include:
  - **Scope statement** (what is in / out for that slice)
  - **Traceability**: PRD IDs + Epic/Story references
  - **Acceptance criteria** (binary, testable) for the slice outcome
  - **Evidence expectation** (e.g., PR link, demo artifact, test output, screenshot, doc update)
- Chunks must respect `UNIVERSAL_PHASE_GATES.md` (human review between phases; no automatic chaining).
...
**§3 — *Every Epic must deliver a coherent business or user outcome, even if it has dependencies on other Epics***
...
Personas must be from the PRD handoff block persona list only. Do NOT invent personas.
...
Preserve every ID exactly as it appears in the PRD (canonical IDs appear in `[PRD_HANDOFF_BLOCK]`).
...
**§9 — Reject Padding**
Do NOT add features, Stories, or Epics not in the `[PRD_HANDOFF_BLOCK]` scope list.
...
4. PRD `[PRD_HANDOFF_BLOCK]` — Authoritative scope
...
**When you encounter a requirement tagged [ASSUMPTION] or [THRESHOLD NEEDED] in the PRD handoff block:**
...
**Q1:** Does this Epic correspond to an FR or NFR ID from the `[PRD_HANDOFF_BLOCK]` scope list?

- If NO → Do not create the Epic. Log it in "Out of Scope Dependencies" section.
- If YES → Proceed to Q2.

**Q2:** Does this Epic imply any logical dependencies or prerequisite features beyond the `[PRD_HANDOFF_BLOCK]` scope?

- If YES → Add them to "Out of Scope Dependencies" section (do not create Epics for them).
- If NO → Proceed to Q3.

**Q3:** Does the Epic description or any of its User Stories exceed the scope ceiling statement in the PRD handoff block?
...
- **Delivery Slices / Chunks (Mandatory):**
  - Slice 1: [Name] — scope, PRD IDs, acceptance criteria, evidence
  - Slice 2: [Name] — scope, PRD IDs, acceptance criteria, evidence
  - (3–8 slices total; ordered; small and reviewable)
...
- Verify: Every PRD ID from the `[PRD_HANDOFF_BLOCK]` appears
 - Verify: Every PRD ID from the `[PRD_HANDOFF_BLOCK]` appears
- ***If output size risks truncation, generate in ordered chunks: Epics 1–3, 4–6, 7–10, then final matrix and handoff block keeping similar subjects together***

### CONTEXT update block (embedded; do NOT create a new file)

```
# CONTEXT — Development Pipeline State (embedded in EPIC output)
...
- [ ]  `[PRD_HANDOFF_BLOCK]` read first and scope list extracted
- [ ]  Scope ceiling statement referenced for every Epic boundary decision
- [ ]  Every Epic cites at least one PRD ID from the handoff scope list (no invented IDs)
...
Process the PRD provided below. Follow all constraints above. Produce output matching the structure in EPIC_Document_Template_Compact.md. Include no blanks — ***Do not leave any section empty. If no applicable content exists, explicitly write “None identified.***.
...
Generate the Epic document now or read the content from the PRD file.