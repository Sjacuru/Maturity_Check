"""
A3 (Ação 3): Inspect retrieval quality for Ação 3 against a real indexed case.

Prints, for each Expected Product (3a) and rio_hints:
  - The exact BM25 FTS5 query string (from data/retrieval_profiles/acao_03.json)
...
Read-only: does not extract, index, or mutate anything.

Usage:
    python scripts/a3_inspect_retrieval_acao3.py [process_number]
"""
...
def inspect_queries() -> None:
    ...
    if rio_acao:
        ...
        print("\n[rio_hints] data/rio_manual/acao_03.json não existe ainda")
...
def main() -> None:
    ...
    print(f"A3 — Retrieval Inspection for Ação {ACAO_ID} (caso real, sem corpus sintético)")
    inspect_queries()
